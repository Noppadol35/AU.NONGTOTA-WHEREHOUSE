    ...
    static async findOrCreateCustomer(customerData) {
        try {
            ...
            if (customer) {
                ...
                console.log(`อัพเดทลูกค้า: ${customer.name} (${customer.licensePlate})`);
            }
            else {
                // สร้างลูกค้าใหม่
                ...
                console.log(`สร้างลูกค้าใหม่: ${customer.name} (${customer.licensePlate})`);
            }
            return customer;
        }
        catch (error) {
    ...
    static async fixJobOrderCustomers() {
        try {
            ...
            for (const jobOrder of jobOrders) {
                if (!jobOrder.customerName || !jobOrder.licensePlate)
                    continue;
                try {
                    // หาหรือสร้างลูกค้า
                    const customer = await this.findOrCreateCustomer({
                        name: jobOrder.customerName,
                        licensePlate: jobOrder.licensePlate,
                    ...
                    if (jobOrder.customerId !== customer.id) {
                        await prisma_1.prisma.jobOrder.update({
                            where: { id: jobOrder.id },
                            data: { customerId: customer.id },
                        });
                        fixedCount++;
                        console.log(`แก้ไข JobOrder ${jobOrder.id}: customerId ${jobOrder.customerId} -> ${customer.id}`);
                    }
                }
                catch (error) {
    ...
    static async getAllCustomersWithStats() {
        try {
            ...
            const customerPromises = customers.map(async (customer) => {
                ...
                const lastVisit = customer.jobOrders.length > 0
                    ? customer.jobOrders.sort((a, b) => new Date(b.createdAt).getTime() -
                        new Date(a.createdAt).getTime())[0]?.createdAt || customer.createdAt
                    : customer.createdAt;
                // หาบริการที่ใช้บ่อย
