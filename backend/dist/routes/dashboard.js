...
// GET /dashboard/stats
router.get("/stats", session_1.sessionRequired, async (req, res) => {
    try {
...
router.get("/recent-activities", session_1.sessionRequired, async (req, res) => {
    try {
        const activities = [];
...
router.get("/top-products", session_1.sessionRequired, async (req, res) => {
    try {
        const products = await prisma.product.findMany({
            where: {
                branchId: req.user?.branchId || 1,
