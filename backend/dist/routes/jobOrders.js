...
router.get("/consumers", session_1.sessionRequired, async (_req, res) => {
    try {
...
router.get("/next-sequence", session_1.sessionRequired, async (req, res) => {
    try {
        const licensePlate = req.query.licensePlate?.trim();
        if (!licensePlate) {
...
// GET /job-orders/:id
router.get("/:id", session_1.sessionRequired, async (req, res) => {
    try {
...
router.post("/", session_1.sessionRequired, async (req, res) => {
    try {
        const { jobNumber, customerName, phoneNumber, carType, licensePlate, issueDetail, jobDetail, } = req.body;
        if (!jobNumber || !customerName || !phoneNumber || !carType || !licensePlate) {
            return res.status(400).json({ message: "missing required fields" });
        }
        // หาหรือสร้างลูกค้าใหม่อัตโนมัติ
        const customer = await customerService_1.CustomerService.findOrCreateCustomer({
            name: customerName,
...
router.put("/:id", session_1.sessionRequired, async (req, res) => {
    try {
        ...
        const { jobNumber, customerName, phoneNumber, carType, licensePlate, issueDetail, jobDetail, status, } = req.body;
        const data = {};
...
router.post("/:id/stock-out", session_1.sessionRequired, async (req, res) => {
    const idParam = req.params.id;
    if (!idParam)
        return res.status(400).json({ message: "missing id parameter" });
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            for (const item of items) {
                ...
                if (product.stockQuantity < qty)
                    throw new Error(`not enough stock for ${product.sku}`);
...
router.delete("/:id/items/:itemId", session_1.sessionRequired, async (req, res) => {
    const idParam = req.params.id;
    const itemIdParam = req.params.itemId;
    if (!idParam || !itemIdParam)
        return res.status(400).json({ message: "missing id parameter" });
    const jobOrderId = parseInt(idParam, 10);
    const itemId = parseInt(itemIdParam, 10);
    if (isNaN(jobOrderId) || isNaN(itemId))
        return res.status(400).json({ message: "invalid id" });
    try {
        const result = await prisma.$transaction(async (tx) => {
            const jobOrder = await tx.jobOrder.findUnique({ where: { id: jobOrderId } });
...
router.patch("/:id/complete", session_1.sessionRequired, async (req, res) => {
    try {
        const idParam = req.params.id;
        if (!idParam) {
            return res.status(400).json({ message: "missing id parameter" });
        ...
        const { paymentMethod, totalAmount, completedAt, subtotal, laborCost, vatAmount, grandTotal } = req.body;
        ...
        const bill = await prisma.bill.create({
            data: {
                ...
                paymentStatus: "PAID",
                paidAt: new Date(),
                createdBy: req.user?.id || 1, // Get user ID from session
...
router.get("/:id/bill", session_1.sessionRequired, async (req, res) => {
    try {
        const idParam = req.params.id;
        if (!idParam) {
            return res.status(400).json({ message: "missing id parameter" });
