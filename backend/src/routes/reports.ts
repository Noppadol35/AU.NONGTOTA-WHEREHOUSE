...
const sessionRequired = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
...
router.get("/low-stock", sessionRequired, async (req, res) => {
  try {
    ...
    const criticalProducts = products.filter(
      (p) => p.stockQuantity <= (p.minStockLevel || 10) * 0.3
    );
    const warningProducts = products.filter(
      (p) =>
        p.stockQuantity > (p.minStockLevel || 10) * 0.3 &&
...
router.get("/summary", sessionRequired, async (req, res) => {
  try {
    ...
    const topMovingCount = await prisma.stockTransaction.count({
      where: {
        type: "SALE",
    ...
    const todayJobs = await prisma.jobOrder.count({
      where: {
        status: "COMPLETED",
        createdAt: {
          gte: today,
    ...
    const todayNewCustomers = await prisma.customer.count({
      where: {
        createdAt: {
          gte: today,
          lt: tomorrow,
    ...
    const todayProductsSold = await prisma.stockTransaction.aggregate({
      where: {
        type: "SALE",
        createdAt: {
          gte: today,
