...
async function computeNextSku(prefixRaw: string): Promise<string> {
...
router.put("/:id", sessionRequired, async (req, res) => {
    try {
        ...
        if (typeof barcode !== "undefined") data.barcode = barcode?.trim() || null;
        ...
        if (typeof costPrice !== "undefined") data.costPrice = costPrice;
        if (typeof sellPrice !== "undefined") data.sellPrice = sellPrice;
        if (typeof minStockLevel !== "undefined") data.minStockLevel = minStockLevel;
        if (typeof branchId !== "undefined") data.branchId = branchId;
        if (typeof stockQuantity !== "undefined") data.stockQuantity = stockQuantity;
...
router.get("/search", sessionRequired, async (req, res) => {
    try {
...
router.get("/next-sku", sessionRequired, async (req, res) => {
    try {
        const prefixRaw = (req.query.prefix as string | undefined) ?? "";
...
router.post("/:id/stock-in", sessionRequired, async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (Number.isNaN(id))
            return res.status(400).json({ message: "invalid id" });

        const { quantity, costPrice, notes } = req.body as {
            quantity: number;
            costPrice?: number;
            notes?: string;
        };
